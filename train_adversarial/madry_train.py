...
parser.add_argument('--pgd-steps', default=10, type=int, help='')
parser.add_argument('--pgd-stepsize', default=2.0/255, type=float, help='')
...
def main():
    ...
    if args.cifar_type == 10:
        ...
        saddle_train_loader = get_full_cifar10_train_loaders(train_batch_size=args.batch_size) 
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        saddle_train(saddle_train_loader, model, criterion, optimizer, epoch)
...
def saddle_train(trainloader, model, criterion, optimizer, epoch):
    ...
    for i, (input_orig, target) in enumerate(trainloader):
        ...
        input_adv, _ = pgd_attack(model, input_orig, target, random_start=True, num_steps=args.pgd_steps, step_size=args.pgd_stepsize)
        ...
        prec = accuracy(output_adv, target)[0]
        top1.update(prec.item(), input_adv.size(0))
