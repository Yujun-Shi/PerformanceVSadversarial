...
def visual_fisher(model, device, loader, num_classes):
    ...
    num_batches = 0
    #Avg_fisher = torch.zeros(28*28, 28*28).type(torch.cuda.FloatTensor)
    Avg_F_norm = 0
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        data, target = data.to(device), target.to(device)
        data_adv,_ = pgd_attack(model, data, target, num_steps=10, step_size=2.0/255, random_start=True, num_classes=10, epsilon=8.0/255)
        optimizer.zero_grad()
